...
from base_models import Similarity_matrix
from tensorflow.keras.layers import Input, Concatenate, Dense
from tensorflow.keras.layers import GRU, LSTM, GRUCell
from tensorflow.keras.layers import Dropout, LSTMCell, RNN, BatchNormalization, LayerNormalization
    ...
    def get_callbacks(self, learning_scheduler, model_path):
        ...
        if learning_scheduler:
            ...
            if 'early_stop' in learning_scheduler:
                default_params = {'monitor': 'val_accuracy',
            ...
            if 'checkpoint' in learning_scheduler:
                default_params = {'filepath': model_path, 'monitor': 'val_accuracy',
...
class PVIM(ActionPredict):
    ...
    def __init__(self,
        ...
        assert self._backbone in ['c3d', 'i3d'], 'Incorrect backbone {}! Should be C3D or I3D'.format(self._backbone)
        self._3dconv = C3DNet if self._backbone == 'c3d' else I3DNet
        self._Similarity_matrix = Similarity_matrix # TBAM
    ...
    def get_data(self, data_type, data_raw, model_opts):
        ...
        for d_type in model_opts['obs_input_type']:
            if 'local' in d_type or 'context' in d_type:
                ...
                box = data['box']
                speed = data['speed']
                u_c = (box[:,:,2]-box[:,:,0])/2+box[:,:,0]
                v_c = (box[:,:,3]-box[:,:,1])/2+box[:,:,1]
                dis = tf.sqrt(tf.square(u_c)+tf.square(v_c))
                sin_theta = v_c / dis
                width = 1080
                height = 1920
                height_middle = height/2
                u_si = abs(height_middle - u_c)
                v_si = abs(width - v_c)
                features = np.dstack((u_si, v_si, sin_theta, speed))
                feat_shape = features.shape[1:]
            else:
                features = data[d_type]
                feat_shape = features.shape[1:]
    ...
    def get_model(self, data_params):
        ...
        if len(encoder_outputs) > 1:
            ...
            for i, enc_out in enumerate(encoder_outputs):
                matrix = self._Similarity_matrix(enc_out,enc_out,enc_out) # TBAM
                x = Activation('relu',name = None)(matrix)
                att_enc_out.append(x)

            # The following part is Information Integration Module
            # concat pedestrian branch
            list_p = [att_enc_out[1],att_enc_out[2]]
            x_p = Concatenate(name='concat_modalities_pedestrian', axis=1)(list_p)
            
            # concat pedestrian-vehicle branch
            list_vp = [att_enc_out[0],att_enc_out[3]]
            x_vp = Concatenate(name='concat_modalities_vehicle_pedestrian', axis=1)(list_vp)
            
       
            x_p = tf.transpose(x_p,[0,2,3,1]) 
            x_vp = tf.transpose(x_vp,[0,2,3,1]) 
            x_v = tf.transpose(att_enc_out[4],[0,2,3,1]) 
            
            # fuse pedestrian branch by CBA
            x_p = Conv2D(filters=16, kernel_size=3, strides=1, padding='same', activation='relu')(x_p) 
            x_p = BatchNormalization()(x_p)
            x_p = Activation('relu',name = None)(x_p)
            
            # fuse pedestrian-vehicle branch by CBA
            x_vp = Conv2D(filters=16, kernel_size=3, strides=1, padding='same', activation='relu')(x_vp) 
            x_vp = BatchNormalization()(x_vp)
            x_vp = Activation('relu',name = None)(x_vp)
            
            # fuse vehicle branch by CBA
            x_v = Conv2D(filters=16, kernel_size=3, strides=1, padding='same', activation='relu')(x_v)
            x_v = BatchNormalization()(x_v)
            x_v = Activation('relu',name = None)(x_v)
            
            # concat all branches
            list_x = [x_p,x_vp,x_v]
            x = Concatenate(name='concat_all_modalities', axis=-1)(list_x)
        ...
        model_output = Dense(512, use_bias=False, activation='relu')(x)
