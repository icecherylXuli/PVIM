...
def C3DNet(freeze_conv_layers=False, weights=None,
    ...
    model = Conv3D(64, 3, activation='relu', padding='same', name='conv1b')(input_data)
    ...
    model = Conv3D(128, 3, activation='relu', padding='same', name='conv2b')(model)
    model = MaxPooling3D(pool_size=(2, 2, 2), strides=(2, 2, 2), padding='valid', name='pool2')(model)
    # 3rd layer group
    model = Conv3D(256, 3, activation='relu', padding='same', name='conv3')(model)
    ...
    # 4th layer group
    model = Conv3D(512, 3, activation='relu', padding='same', name='conv4')(model)
    ...
    # 5th layer group
    model = Conv3D(256, 3, activation='relu', padding='same', name='conv5')(model)
    model = Conv3D(256, 3, activation='relu', padding='same', name='conv5b')(model)
...
def attention(inputs_query,inputs_key,inputs_value,frame_size = 16, modim = 512,scale=64, att_dropout=None, attn_mask = None):

    scale = 16
    
    inputs_key_t = tf.transpose(inputs_key,[0,1,3,2])
    scores = tf.matmul(inputs_query, inputs_key_t) 
    scores = scores / np.sqrt(scale)
    scores = tf.nn.softmax(scores)
    scores = Dropout(0.5)(scores)
    
    return scores
    
    
def Similarity_matrix(inputs_query,inputs_key,inputs_value,num_heads=4, model_dim=512):
    
    input_size = 512

    fc_q = Dense(units = model_dim, input_shape=(input_size,))
    fc_k = Dense(units = model_dim, input_shape=(input_size,))
    fc_b = Dense(units = model_dim, input_shape=(input_size,))
    linear_q = fc_q(inputs_query)
    linear_k = fc_k(inputs_key)
    linear_v = fc_b(inputs_value)
    
    shape_size = [16,num_heads, model_dim//num_heads]
    query = Reshape(shape_size)(linear_q)
    key = Reshape(shape_size)(linear_k)
    value = Reshape(shape_size)(linear_v)
    
    query = tf.transpose(query,[0,2,1,3])
    key = tf.transpose(key,[0,2,1,3])
    value = tf.transpose(value,[0,2,1,3])
    
    matrix = attention(query,key,value)
